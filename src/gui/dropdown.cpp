...
DDOption::DDOption(const sf::Vector2f& pos, const std::string& _value, const sf::Font& font, float width) {
...
bool DDOption::handle_event(const sf::Event& event) {
    switch (event.type) {
...
void DDOption::draw(sf::RenderTarget& target) const {
    target.draw(shape);
    target.draw(label);
}
...
Dropdown::Dropdown(const sf::Vector2f& pos, const std::vector<std::string>& values, const sf::Font& font, float width, bool dynwidth) {
    ...
    for (const auto& v : values) {
        options.push_back(DDOption(pos + sf::Vector2f(0, h), v, font, width));
        h += 30.f;
    }
}


void Dropdown::set_position(const sf::Vector2f& pos) {
    float h = 0.f;
    for (auto& op : options) {
